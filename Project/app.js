...
function showTransformForm() {
  ...
  decreaseContrastForm.addEventListener("submit", (e) => {
    e.preventDefault()
    var od = document.getElementById("od2").value
    var ob = document.getElementById("ob2").value
    var td = document.getElementById("td2").value
    var tb = document.getElementById("tb2").value
    decreaseContrast(Number(od),Number(ob),Number(td),Number(tb))
  });
  ...
  function increaseContrast(od, ob , td ,tb) {
    ...
    var val;
    const c1 = td/od
    const c2= (tb-td)/(ob-od)
    const c3= (255-tb)/(255-ob)
    const c= c2*od - td
    const c4= tb - c3*ob
    ...
    for (i = 0; i < img.width * img.height * 4; i += 4) {
      if ( rgba[i] <= od ) {
        val = Math.round(c1* rgba[i]);
      }
      else if ( rgba[i] >= ob ) {
        val = Math.round(c3* rgba[i] + c4);
      }
      else {
        val = Math.round(c2* rgba[i] - c);
      }
      if(val>255){
        val=255;
      }
  ...
  function decreaseContrast(od, ob , td ,tb) {
    const img = document.getElementById("inputImage");
    const canvas = document.getElementById("resultImage");
    const ctx = canvas.getContext('2d');
